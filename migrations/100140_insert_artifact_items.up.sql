...
VALUES 
    (
        (SELECT id FROM items WHERE name = 'Amber Casement'),
        '1d6 + 4',
        '1-4 in 1d6',
    ...
    (
        (SELECT id FROM items WHERE name = 'Living Armour Sheath'),
        '1d6 + 1',
...
INSERT INTO artifacts_pool_modifiers (
    artifact_id,
    pool_type,
    modifier_value,
    edge_value
...
VALUES 
    (
        (SELECT item_id FROM artifacts WHERE item_id = (SELECT id FROM items WHERE name = 'Living Armour Sheath')),
        'ARMOUR',
        2,
        0
    );